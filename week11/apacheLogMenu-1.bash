...
ioc="/home/champuser/SYS320-01/week11/ioc.txt"
...
function suspiciousVisitors(){
	echo "Suspicious IPs found:"
	cat "$ioc" | while read -r indicator;
	do
		cat "$logFile" | grep "$indicator"
	done | cut -d ' ' -f 1 | sort | uniq -c | sort -rn
}
...
do
	...
	# Display a message, if an invalid input is given
		...
		echo "Invalid Input. Choose a number between 1-7"
	fi
done
